...
const InvoicePreview = ({ order, settings }) => {
    ...
    const displayId = order.orderId ? String(order.orderId) : (order.id ? String(order.id) : 'Новый');

    // Получаем имя клиента корректно
    const clientName = order.clientName || order.client?.name || order.selectedClient?.name || 'Не указан';
        ...
        <div className="bg-white max-w-[210mm] min-h-[297mm] mx-auto p-8 shadow-lg print:shadow-none print:m-0 print:w-full print:h-auto print:min-h-0 flex flex-col justify-between" id="invoice">
            ...
            <div>
                    ...
                    <div className="flex flex-wrap gap-6 items-center text-gray-600">
                        ...
                        {showSubtotal && (
                            <div className="flex gap-2 items-center">
                                <span className="text-gray-400 font-medium">Сумма:</span>
                        ...
                        {hasPrepayment && (
                            <div className="flex gap-2 items-center bg-gray-50 px-2 py-0.5 rounded border border-gray-200">
                                ...
                                <span className="font-bold text-gray-800">{prepaymentOriginal} {prepaymentCurrency}</span>
                            </div>
                        )}
                    </div>
                ...
                {hasPrepayment && (
                    <div className="grid grid-cols-2 gap-16 mt-16 mb-8">
                        <div><div className="border-b border-gray-400 mb-1"></div><div className="text-center text-[10px] text-gray-400 uppercase tracking-wide">Представитель бренда</div></div>
                        <div><div className="border-b border-gray-400 mb-1"></div><div className="text-center text-[10px] text-gray-400 uppercase tracking-wide">Клиент</div></div>
                    </div>
                )}
            </div>
            ...
            <div className="mt-auto">
                ...
                <div className="pt-4 border-t border-gray-200 flex justify-between items-center text-[10px] text-gray-400">
                    <div className="flex items-center gap-2"><span>Создано в</span><img src={`${IMG_URL}/proboot-invoice.png`} alt="ProBoot" className="h-4 opacity-70" onError={(e) => e.target.style.display = 'none'} /></div>
