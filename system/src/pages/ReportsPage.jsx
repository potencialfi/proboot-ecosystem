...
const ReportsPage = ({ orders = [], clients = [], settings, onEditOrder, onReload }) => {
        ...
        (orders || []).forEach(order => {
            ...
            if (!clientMap[groupingId]) {
                ...
                clientMap[groupingId] = {
                    id: groupingId,
                    orderIds: [], 
                    name: dbClient.name || order.clientName || 'Неизвестный',
                    phone: dbClient.phone || order.clientPhone || '',
                    city: dbClient.city || order.clientCity || '',
                    note: order.note || '',
                    totalQty: 0,
                    totalDiscount: 0,
                    prepayment: 0,
            ...
            if (client.name === 'Неизвестный' && order.clientName && order.clientName !== 'Неизвестный') client.name = order.clientName;
            ...
            (order.items || []).forEach(item => {
                ...
                if (!modelsMap[modelKey]) {
                    modelsMap[modelKey] = { key: modelKey, sku: sku, color: color, sizes: new Set(), totalQty: 0, sizeTotals: {} };
                }
    ...
    const handleDraftChange = (modelKey, size, val) => {
        setDraftValues(prev => ({ ...prev, [`${modelKey}-${size}`]: val.replace(/\D/g, '') }));
    };
    const getDraftTotal = (modelKey) => {
        let sum = 0;
        Object.entries(draftValues).forEach(([k, v]) => { if (k.startsWith(`${modelKey}-`) && v) sum += parseInt(v); });
        return sum;
    };
    ...
    const requestClearDrafts = () => {
        ...
        setConfirmModal({ isOpen: true, type: 'clear_draft', title: 'Очистить черновик?', message: 'Это удалит все данные из черновика. Продолжить?' });
    };
    const requestDeleteRow = (client) => {
        setConfirmModal({ isOpen: true, type: 'delete_row', payload: client, title: 'Удалить заказ?', message: `Вы действительно хотите удалить все заказы клиента ${client.name}?` });
    };
    const handleConfirmAction = async () => {
        if (confirmModal.type === 'clear_draft') setDraftValues({});
        else if (confirmModal.type === 'delete_row') {
            try { await Promise.all(confirmModal.payload.orderIds.map(id => apiCall(`/orders/${id}`, 'DELETE'))); if (onReload) onReload(); } 
    ...
    const handleDownloadRow = async (client) => {
        try {
            ...
            rows.push([], ["Итого пар:", client.totalQty], ["Скидка:", client.totalDiscount], ["Предоплата:", Number(client.prepayment.toFixed(2))], ["Сумма:", client.totalSum]);
    ...
    const handleExport = async () => {
        try {
            ...
            const bodyRows = clientsData.map(c => {
                const row = [c.name, c.phone, c.city, c.note, c.totalQty, c.totalDiscount, Number(c.prepayment.toFixed(2)), c.totalSum];
                uniqueModels.forEach(model => { model.sortedSizes.forEach(size => row.push(c.items[model.key]?.[size] || "")); });
                return row;
            });
            const ws = XLSX.utils.aoa_to_sheet([headerRow1, headerRow2, ...bodyRows]);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Отчет");
            XLSX.writeFile(wb, `Report_Full.xlsx`);
        } catch (e) { alert("Ошибка: " + e.message); }
    };

    return (
        <div className="page-container relative h-full flex flex-col p-6 bg-slate-50 overflow-hidden">
            <ConfirmModal isOpen={confirmModal.isOpen} onClose={() => setConfirmModal({...confirmModal, isOpen: false})} onConfirm={handleConfirmAction} title={confirmModal.title} message={confirmModal.message} />
            ...
            <div className="page-header-card shrink-0 mb-0">
                ...
                <div className="flex gap-3"><Button onClick={handleExport} variant="secondary" icon={Download}>Экспорт</Button></div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap text-sm">
                        <thead className="bg-slate-50 sticky top-0 z-[60] shadow-md">
                            ...
                            <tr className="h-10 bg-yellow-50">
                                ...
                                <th className="sticky top-12 left-[100px] z-[70] bg-yellow-100 border-b border-r border-gray-200 text-right px-3 font-bold text-yellow-800 uppercase text-xs">
                                    <div className="flex items-center justify-end gap-3 h-full w-full">
                                        <span>Черновик</span>
                                        {totalDraftSum > 0 && (<><div className="bg-white/60 px-1.5 py-0.5 rounded border border-yellow-300/50"><span className="text-yellow-900 font-black">{totalDraftSum}</span></div><button onClick={requestClearDrafts} className="bg-white hover:bg-red-50 p-1 rounded border border-yellow-300 hover:border-red-200 text-yellow-600 hover:text-red-500 transition-colors shadow-sm"><Eraser size={14}/></button></>)}
                                    </div>
                                </th>
                            ...
                            <tr className="h-8 bg-gray-100">
                                ...
                                {uniqueModels.map((model, mIdx) => (
                                    model.sortedSizes.map((size, idx) => (
                                        <th key={`total-${model.key}-${size}`} className={`sticky top-[88px] p-1 border-b border-r border-gray-200 text-center text-[10px] text-gray-500 font-bold bg-gray-100 ${idx===0 ? 'border-l-2 border-l-gray-300' : ''}`}>{model.sizeTotals[size] || 0}</th>
                                    ))
                                ))}
                            </tr>
                            <tr className="h-10 bg-white">
                                ...
                                <th className="sticky top-[120px] bg-white border-b border-gray-300 text-center min-w-[80px]"><div className="text-[10px] text-black font-bold">Кол-во</div><div className="text-sm font-bold text-indigo-600">{grandTotalPairs}</div></th>
                                ...
                                {uniqueModels.map((model) => (
                                    model.sortedSizes.map((size, idx) => (
                                        <th key={`size-${model.key}-${size}`} className={`sticky top-[120px] p-1 border-b border-r border-gray-200 text-center text-xs text-gray-600 min-w-[40px] bg-white ${idx===0 ? 'border-l-2 border-l-gray-300' : ''}`}>{size}</th>
                                    ))
                                ))}
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {clientsData.map(client => (
                                <tr key={client.id} className="hover:bg-blue-50/30 transition-colors group">
                                    <td className="p-2 border-r border-gray-200 sticky left-0 bg-white group-hover:bg-blue-50 z-30 flex gap-1 justify-center items-center h-full shadow-r"><button onClick={() => handleDownloadRow(client)} className="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded"><Download size={16}/></button><button onClick={() => handleEditRow(client)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded"><Edit size={16}/></button><button onClick={() => requestDeleteRow(client)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded"><Trash2 size={16}/></button></td>
                                    <td className="p-3 border-r border-gray-200 font-bold text-gray-800 sticky left-[100px] bg-white group-hover:bg-blue-50 z-30 shadow-r">{client.name}</td>
